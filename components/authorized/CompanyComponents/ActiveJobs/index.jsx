...
export default function ActiveJobs({ filterType = 'active' }) {
  ...
  const fetchJobs = async () => {
    try {
      ...
      const filteredJobs = jobsWithApplicants.filter(job => job.active === (filterType === 'active'));
        ...
        <div className={styles.activeJobs__container}>
          <div className={styles.section__header}>
            <h2 className={styles.section__title}>
              {filterType === 'active' ? 'Active Jobs' : 'Inactive Jobs'}
            </h2>
          </div>
          <p className={styles.no__jobs}>No {filterType === 'active' ? "active" : "inactive"} jobs found.</p>
        </div>
      </section>
    );
  }
